...
class EMSSA(nn.Module):
    def __init__(self, channels):
        ...
        self.conv1_mid = nn.Conv2d(in_channels=channels // 4, out_channels=channels // 4, kernel_size=1, stride=1,
                                   padding=0)
        self.conv1_down = nn.Conv2d(in_channels=channels, out_channels=channels // 4, kernel_size=1, stride=1,
                                    padding=0)
    ...
    def forward(self, x0):
        ...
        out = torch.add(self.conv1(torch.cat((s0, s1, s2, s3), dim=1)), self.conv1_up(x))
...
class RFDB_WOA(nn.Module):
    def __init__(self, channels):
        ...
        self.conv1 = nn.Conv2d(in_channels=channels, out_channels=channels, kernel_size=1, stride=1, padding=0)
        self.conv1_half = nn.Conv2d(in_channels=channels, out_channels=channels // 2, kernel_size=1, stride=1,
                                    padding=0)
    ...
    def forward(self, x):  # 48 channels
        ...
        x6 = self.ReLU(self.conv3_half(self.ReLU(torch.add(x4, self.conv3(x4)))))  # 24 channels
...
class upsampler(nn.Module):
    def __init__(self, channels, upscale_factor, mid_channels=54):
        ...
        self.BSConv1_2 = BSConvU(channels // 4, channels, kernel_size=3, padding=1)  # x2
        self.BSConv_3 = BSConvU(channels, mid_channels, kernel_size=3, padding=1)  # x3
        self.BSConv1_3 = BSConvU(mid_channels // 9, channels, kernel_size=3, padding=1)  # x3
...
with torch.no_grad():
    model = MSDAN(channels=48, num_DFEB=8, upscale_factor=2)
    input1 = torch.rand(1,3,256,256)
    state_dict = torch.load('MSDAN_x2.pth')
